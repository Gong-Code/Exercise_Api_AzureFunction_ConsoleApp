            ...
            ILogger log)
        {
            ...
            if (!string.IsNullOrEmpty(data.DeviceId))
            {
                try
                {
                    ...
                    if (device != null)
                    {
                        return new OkObjectResult($"HostName=systemDev-IotHub.azure-devices.net;DeviceId={device.Id};SharedAccessKey={device.Authentication.SymmetricKey.PrimaryKey}");
                    }
